...
panelIndex = 1;
numPlots = 5;
load('p10b_switchingWithk_small.mat') % load data

% for-loop to make individual subplots
for i = 1:numPlots
    ax1(i) = axes(f); % make axis
    plot(ts{i},As{i},'linewidth',2) % plot time series data
    ...
    histogram(As{i},nHistBins,'Orientation','horizontal') % plot histogram

    % make plot pretty
    % set(ax2(i),'Box','off','XColor','none','Ycolor','none','FontSize',14) 
    set(ax2(i),'XTickLabel',[],'YTickLabel',[],'XTick',[])
    title(['{\itk} = ' num2str(param{i}.k)],'Rotation',270,'Units','normalized','Position',[1.05,0.5],'FontWeight','normal')
    if i ~= numPlots
        ax1(i).XTickLabel = [];
    end
end
...
for i = 1:numPlots
    ...
    % set(ax2(i),'Box','off','XColor','none','Ycolor','none','FontSize',14)
    set(ax2(i),'XTickLabel',[],'YTickLabel',[],'XTick',[])
    title(['{\itL} = ' num2str(length(param{i}.A0))],'Rotation',270,'Units','normalized','Position',[1.05,0.5],'FontWeight','normal')
    if i ~= numPlots
        ax1(i).XTickLabel = [];
    end
end
