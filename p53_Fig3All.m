...
axPSDvsk = axes(myFig);
for i = 1:4
    loglog(axPSDvsk,Fig3Data.A.freq,Fig3Data.A.psdk{i},'linewidth',2) % plot psd from sim
    hold on
    loglog(axPSDvsk,Fig3Data.A.freq,lorentzFunction(Fig3Data.A.lorentzFitVals{i},Fig3Data.A.freq),'k','linewidth',2) % plot lorentz fit of psd
end
...
yyaxis left
loglog(axStatsVsk,Fig3Data.B.k,Fig3Data.B.variance,'o-','linewidth',2)
xlabel('{\itk}')
ylabel('Activity Variance (<A^2> - <A>^2)')
text(10^-4,0.8*10^-2,'Mean Activity = 0.5','FontSize',14)

yyaxis right
loglog(axStatsVsk,Fig3Data.B.k,Fig3Data.B.lorentzFitVals,'o-','linewidth',2)
ylabel('Characteristic Frequency (1/s)','Rotation',-90)
xlim([Fig3Data.B.k(1),Fig3Data.B.k(end)])
...
L = cToLFunc(defaultParams,M,Fig3Data.C.c);
varianceVals = Fig3Data.C.variance;
...
LVals = cToLFunc(defaultParams,M,cVals);
...
loglog(Fig3Data.D.freq,Fig3Data.D.untunedPSD,'linewidth',2)
hold on
loglog(Fig3Data.D.freq,Fig3Data.D.tunedPSD,'linewidth',2)
xlabel('Frequency (1/s)')
ylabel('Power Spectral Density')
set(axTunedVsAdapt,'fontsize',15,'OuterPosition',[1/3,0,1/3,0.45],'XScale','log','YScale','log','Box','on')
xlim([10^-4,10])
ylim([10^-5,10])
loglog(Fig3Data.D.adaptingFreq,Fig3Data.D.adaptingPSD,'linewidth',2)
legend(axTunedVsAdapt,'Untuned','Tuned','Adapting','location','northeast')
legend('boxoff')

%% 3E: Plot Adapting PSDs
axAdaptingPSD = axes(myFig);
loglog(Fig3Data.D.adaptingFreq, Fig3Data.E.slowAdaptPSD,...
    Fig3Data.D.adaptingFreq,Fig3Data.E.mediumAdaptPSD,...
    Fig3Data.D.adaptingFreq,Fig3Data.E.fastAdaptPSD,'linewidth',2)
xlabel('Frequency (1/s)')
ylabel('Power Spectral Density')
xlim([10^-4,10])
ylim([10^-5,10])
...
plot(Fig3Data.F.slowAdapt_ts, Fig3Data.F.slowAdapt_As,'linewidth',2);
set(gca,'fontsize',15)
xticklabels({})
xlim([500,520])
ylim([-0.1,1.1])
...
plot(Fig3Data.F.mediumAdapt_ts, Fig3Data.F.mediumAdapt_As,'Color',[0.8500 0.3250 0.0980]*1.15,'linewidth',2);
set(gca,'fontsize',15)
xticklabels({})
xlim([500,520])
ylim([-0.1,1.1])
...
plot(Fig3Data.F.fastAdapt_ts-500, Fig3Data.F.fastAdapt_As,'color',[0.9290 0.6940 0.1250],'linewidth',2);
set(gca,'fontsize',15)
xticklabels({})
xlim([0,20])
ylim([-0.1,1.1])
